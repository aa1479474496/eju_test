...
(function($) {
    ...
    "set" : [function(v,args){return args.replace(/\s+/g, '').replace(/，/g, ',').split(',').indexOf(v) !== -1;}, "请选择{0}其中一个"],
		...
		"regexp" : ['', '请输入正确的格式'],
		"function" : ['', '请输入正确的格式'],
    "yearmonth": [/^([0-9]{4})-((0?[1-9])|(1[0-2]))$/, '请输入年月，格式yyyy-MM'],
    ...
    "select": [function(v, args){
      var arr = v.split(',');
      if (arr[0] == '') {
        arr.shift();
      }
      return arr.length >= parseInt(args, 10);
    }, '请至少选择{0}个'],
	...
	$.validator.tipHandler = function(e, status, msg) {
		...
		if (status == 2) {
			tipele.html('');
            tipele.removeClass('text-danger');
            tipele.removeClass('text-navy');
		// 正确
		} else if (status == 1) {
            tipele.removeClass('text-danger');
            tipele.addClass('text-navy');
            tipele.html('');
		// 错误
		} else {
            tipele.addClass('text-danger');
            tipele.removeClass('text-navy');
            tipele.html(msg);
		}
	}
	...
	$.validator.prototype = {
		submit: function() {
			...
			if (ok == false) {
        $(first).focus();
        if (typeof validator.settings.failHandler == 'function') {
          validator.settings.failHandler(this);
        }
				return false;
			}
