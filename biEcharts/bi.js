window.bi = {
  ...
  isset(obj, k) {
    return this.isObject(obj) && obj.hasOwnProperty(k);
  },
  isEmpty(val) {
    switch (typeof val) {
      case 'string':
  ...
  /**
	 ...
	 * 
	 * @param {*} data
	 * @param {*} args formatData(data, 'max', 'field'); formatData(data, 'min',
	 *            'field'); formatData(data, 'col', 'field', type);
	 *            formatData(data, 'pair', 'filedkey', 'fieldval', val_type);
	 *            formatData(data, 'group', 'field1', 'field2', 'field3', ...);
	 *            formatData(data, 'group', ['field1', 1], ['field2', 2],
	 *            'field3', ...); formatData(data, 'kv', 'k1', 'v1', 'k2', 'v2',
	 *            ...); formatData(data, 'kv', ['k1', 'v1', 't1'], ['k2', 'v2',
	 *            't2'], ...); formatData(data, 'tree', 'TODO');
	 */
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          for (let j = 0; j < keys.length; j++) {
            if (Array.isArray(keys[j])) {
              // 兼容这种模式: formatData(data, 'group', ['field1', 1],['field2',0],
				// ['field3', 1],...);
              row.push(this.formatType(data[i][keys[j][0]], keys[j][1]));
            } else {
              // 兼容这种模式: formatData(data, 'group', 'field1', 'field2',
				// 'field3', ...);
  ...
  /*
	 * Unix时间戳转换成日期格式 FormatDateTime('1497232433000') @param unixTime Unix时间戳
