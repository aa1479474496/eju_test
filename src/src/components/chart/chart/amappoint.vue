        ...
        z-index="999">
          <div class="contentInfo">
            <div class="contentinner">
              <div class="title" v-if="chartInfo.data.query.fields1.length>2">{{marker.data[chartInfo.data.query.fields1[2].title]}}</div>
...
export default {
  ...
  computed: {
    ...
    markerIcon() {
      ...
      let _icon =
        this.options.icon ||
        bi.dash.amapthemes[_theme][this.chartInfo.data.type].icon;
      return _icon;
    }
  },
  methods: {
    ...
    getStyle(marker) {
      ...
      let _styles =
        this.options.styles ||
        bi.dash.amapthemes[_theme][this.chartInfo.data.type].styles;
      let _data = this.options.data;
      let _min = this.options.min;
      let _max = this.options.max;
      let _split = bi.formatSplit(_min, _max, _styles.length);
      let _ret = {};
      let _value = parseInt(
        marker[this.chartInfo.data.query.fields1[2].title],
        10
      );
      for (let i = 0; i < _styles.length; i++) {
        if (_value < _split.min + _split.split * (i + 1)) {
          _ret = {
            background: _styles[i].color,
            width: _styles[i].size + "px",
            height: _styles[i].size + "px"
          };
          break;
        }
      }
